...
const Hero = () => {
  ...
  return <section ref={sectionRef} className="bg-background">

      <div className="section-container py-12 sm:py-16 lg:py-28">
        <div className="mx-auto max-w-4xl text-center">

          {/* Headline */}
          <h1 className={`text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-extrabold tracking-tight text-foreground transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            Your{" "}
          ...
          <div className={`mt-8 sm:mt-10 transition-all duration-700 delay-200 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            <Link to="/vsl" className="inline-flex items-center justify-center rounded-full bg-primary px-8 py-4 text-base sm:text-lg font-semibold text-primary-foreground shadow-lg shadow-primary/30 transition-all hover:bg-primary/90 hover:shadow-xl hover:shadow-primary/40 hover:scale-105">
              Watch Demo
          ...
          <p className={`mt-4 text-xs sm:text-sm text-muted-foreground transition-all duration-700 delay-300 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>21-day free trial · No contracts · Cancel anytime</p>

          {/* Trust Stats - Mobile friendly grid */}
          <div className={`mt-10 sm:mt-12 grid grid-cols-3 gap-4 sm:gap-8 max-w-md sm:max-w-xl mx-auto transition-all duration-700 delay-400 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
